...
async function ensureSchema(sql: Sql = db()) {
  await sql`
    create table if not exists bluesky_fightback_daily_reposts (
...
async function latestOriginalPost() {
  ...
  if (!response.ok) {
    throw new Error(`Fight Back News Bluesky feed returned ${response.status}.`);
  }
  ...
  const candidates = (payload.feed ?? [])
    .filter((item) => {
      ...
      return (
        item.post.author.did === FIGHTBACK_DID &&
        !item.reason &&
...
export async function runDailyFightBackRepost(
  ...
  if (!post) {
    return { ok: false, skipped: true, message: "No Fight Back News post was found." };
  }

  const sql = await ensureSchema();
  const existing = await sql`
    select status, post_uri, repost_uri
    from bluesky_fightback_daily_reposts
  ...
  if (existing[0]?.status === "posted") {
    return {
      ok: true,
      skipped: true,
      message: "Fight Back News was already reposted today.",
  ...
  if (
    ...
    return {
      ok: true,
      skipped: true,
      message: "Fight Back News has not published a newer post yet.",
      postUri: String(existing[0].post_uri),
    };
  }
  if (existing[0]?.status === "posting") {
    return { ok: true, skipped: true, message: "Fight Back News repost is already in progress." };
  }
  ...
  if (prior[0]) {
    await sql`
      insert into bluesky_fightback_daily_reposts(
        local_date,source_did,source_handle,post_uri,post_cid,post_created_at,status,last_error
      ) values(
        ${local.date}::date,${FIGHTBACK_DID},${FIGHTBACK_HANDLE},${post.uri},${post.cid},
    ...
    return {
      ok: true,
      skipped: true,
      message: "Fight Back News has not published a newer post since the last repost.",
      postUri: post.uri,
    };
  }
  ...
  try {
    ...
    await sql`
      update bluesky_fightback_daily_reposts
    ...
    return {
      ok: true,
      skipped: false,
      source: `@${FIGHTBACK_HANDLE}`,
      postUri: post.uri,
      repostUri: repost.uri,
      postCreatedAt: post.record.createdAt ?? null,
    };
  } catch (error) {
    const message = error instanceof Error ? error.message.slice(0, 900) : "Fight Back News repost failed.";
    await sql`
      update bluesky_fightback_daily_reposts

import type { LeadOpportunity, LeadSource } from "@/lib/leads/types";

export type LeadFeed = "website" | "power-bi" | "data-ops" | "partners";
export type OutreachSegment = LeadFeed;
...
export function deriveOutreachSegment(input: {
  ...
  const text = [input.company, input.summary, ...(input.signals ?? []), ...(input.tags ?? [])]
    .filter(Boolean)
    .join(" | ");
...
export function pitchVersionForLead(segment: OutreachSegment, leadId: string) {
  return `${segment}-2026-08-${pitchVariantForLead(leadId).toLowerCase()}`;
}

export function isLeadFeed(value: string | null): value is LeadFeed {
  return value === "website" || value === "power-bi" || value === "data-ops" || value === "partners";
}

export const LEAD_FEEDS: readonly LeadFeed[] = ["website", "power-bi", "data-ops", "partners"];

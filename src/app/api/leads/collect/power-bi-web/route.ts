...
function qualifyJob(result: BraveResult, search: SearchConfig, sourceUrl: string, title: string, description: string, combined: string): PowerBiGigInput | null {
  if (!powerBiPattern.test(combined) || providerPattern.test(combined) || editorialPattern.test(combined)) return null;
  const sourcePublishedAt = parsePublishedAt(result);
  if (!sourcePublishedAt) return null;
  const ageMs = Date.now() - Date.parse(sourcePublishedAt);
  if (ageMs < -300_000 || ageMs > JOB_MAX_AGE_MS) return null;
...
function qualify(result: BraveResult, search: SearchConfig): PowerBiGigInput | null {
  ...
  if (search.useDayFreshness === false || isJobLike(sourceUrl, combined)) {
    return qualifyJob(result, search, sourceUrl, title, description, combined);
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return privateJson({ ok: true, partial: searchErrors.length > 0, source: SOURCE_ID, seen, qualified: gigs.length, stored, expired, socialPlatform: socialSearch.platform, jobBoardSources: jobSearches.map((search) => search.platform), searchCounts, searchErrors, budget });
  } catch (error) {

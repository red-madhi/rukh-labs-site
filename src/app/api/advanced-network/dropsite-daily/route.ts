...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await runDailyDropSiteRepost({ force: request.nextUrl.searchParams.get("force") === "1" });
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Drop Site daily repost failed." }, {
      status: 500,
      headers: { "Cache-Control": "no-store, max-age=0", "X-Robots-Tag": "noindex, nofollow, noarchive, nosnippet" },
    });
  }

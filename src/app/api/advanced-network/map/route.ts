...
function influenceScore(profile: Profile, mutual: boolean) {
  ...
  return Math.round(Math.log10(followers + 10) * 22 + Math.min(24, Math.log2(ratio + 1) * 8) + (mutual ? 35 : 0));
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const enrichedFollowers = followers.map((profile) => {
      ...
      return { profile, mutual, followedBy: Boolean(relation?.followedBy), score: influenceScore(profile, mutual) };
    });

    const startingPool = enrichedFollowers.filter((item) => scope === "all-followers" || item.mutual);
    const selectedStarting = [...startingPool]
      .sort((a, b) => b.score - a.score || (b.profile.followersCount ?? 0) - (a.profile.followersCount ?? 0))
      .slice(0, MAX_STARTING_NODES);

    const targetInputs = Array.from(new Set((body.targets ?? []).map((value) => normalize(String(value))).filter(Boolean))).slice(0, MAX_TARGETS);
    const targetProfiles = (
      await Promise.all(targetInputs.map(async (target) => {
        try {
    ...
    if (targetProfiles.length) {
      const actorTargetRelationships = await getRelationships(actor.did, targetProfiles.map((profile) => profile.did));
      ...
      for (const target of targetProfiles) {
        const relationships = startingDids.length ? await getRelationships(target.did, startingDids) : [];
    ...
    const verifiedBridgeCount = edges.filter((edge) => edge.kind === "verified-target-bridge").length;
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Live network map failed." }, { status: 502 });
  }
}

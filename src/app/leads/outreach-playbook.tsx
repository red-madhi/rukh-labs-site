...
import { Check, Clipboard, Clock3, ExternalLink, Mail, ShieldCheck } from "lucide-react";
      ...
      <div className="mt-5 border-t border-white/8 pt-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
          <div className="border border-white/8 bg-black/20 p-2.5">
            <span className="block text-[8px] font-bold uppercase tracking-[.13em] text-white/25">Instantly avg</span>
            <strong className="mt-1 block text-sm text-white/70">{OUTREACH_RESEARCH.averageReply}</strong>
          </div>
          <div className="border border-white/8 bg-black/20 p-2.5">
            <span className="block text-[8px] font-bold uppercase tracking-[.13em] text-white/25">Instantly top 25%</span>
            <strong className="mt-1 block text-sm text-white/70">{OUTREACH_RESEARCH.topQuartileReply}</strong>
          </div>
          <div className="border border-white/8 bg-black/20 p-2.5">
            <span className="block text-[8px] font-bold uppercase tracking-[.13em] text-white/25">Instantly top 10%</span>
            <strong className="mt-1 block text-sm text-white/70">{OUTREACH_RESEARCH.topDecileReply}</strong>
          </div>
          <div className="border border-white/8 bg-black/20 p-2.5">
            <span className="block text-[8px] font-bold uppercase tracking-[.13em] text-white/25">Replies after step 1</span>
        ...
        <p className="mt-2 text-[9px] leading-4 text-white/24">
          Platform benchmarks use different audiences and denominators. Treat them as directional reference points, never promised results.
        </p>
        ...
        <div className={`mt-2 flex items-start gap-2 border ${accentBorder} bg-black/20 p-3`}>
          <Clock3 className={`mt-0.5 size-3.5 shrink-0 ${accent}`} aria-hidden />
          <div>
            <p className="text-[8px] font-bold uppercase tracking-[.15em] text-white/28">Send strategy</p>
            <p className="mt-1 text-[10px] leading-5 text-white/42">
              {urgent
                ? "Freshness wins here: send the first touch as soon as the contact/source is verified. Do not hold a ≤12-hour job-board or explicit buyer-intent lead for an ideal weekday."
                : "For non-urgent outreach, favor the recipient's morning and Monday–Wednesday. Instantly found Wednesday strongest for engagement; keep follow-ups 3–4 days apart."}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-3 xl:grid-cols-2">
          ...
          <div className="border border-white/8 bg-black/20 p-3">
            ...
            <div className="mt-2 space-y-2 text-[10px] leading-4 text-white/40">
              ...
              <p>Gong CTA study: 304,174 emails; asking for interest instead of time was 2×+ more likely to lead to a booked meeting.</p>
              <p>Instantly: billions of interactions; elite campaigns average &lt;80 words with one CTA; 42% of replies arrive after the first touch.</p>
              <p>Lavender: 231,818 recent emails / ~50k inboxes; mobile-friendly formatting produced 83% more replies on average.</p>
              <p>Belkins/Reply.io: 5.5M emails; personalized subjects moved replies from 3% to 7%, and 2–4 word subjects had the highest opens in that dataset.</p>
            </div>

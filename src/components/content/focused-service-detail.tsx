...
export function FocusedServiceDetail({ service }: { service: FocusedService }) {
  ...
  return (
    <>
      ...
      <Section className="border-y border-white/10 bg-white/[0.02]"><Container className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]"><Reveal><div><Badge tone="gold">{service.evidenceEyebrow}</Badge><h2 className="mt-5 text-3xl font-semibold text-white sm:text-5xl">{service.evidenceTitle}</h2><p className="mt-5 text-lg leading-8 text-white/62">{service.evidenceDescription}</p></div></Reveal><Reveal delay={0.08}><ul className="grid gap-4">{service.approach.map((item) => <li key={item} className="flex items-start gap-3 text-base leading-7 text-white/68"><Check aria-hidden className="mt-1 size-4 shrink-0 text-[#67e8f9]" />{item}</li>)}</ul></Reveal></Container></Section>
      <Section>
        <Container>
          <Reveal>
            <div className="max-w-3xl">
              <Badge tone="blue">Fictional planning example</Badge>
              <h2 className="mt-5 text-3xl font-semibold text-white sm:text-5xl">{service.exampleTitle}</h2>
              <p className="mt-5 text-lg leading-8 text-white/62">{service.exampleDescription}</p>
            </div>
          </Reveal>
          <div className="mt-10 grid gap-4 sm:grid-cols-2">
            {service.exampleItems.map((item, index) => (
              <Reveal key={item.label} delay={(index % 4) * 0.04}>
                <Card className="h-full p-5 sm:p-6">
                  <h3 className="text-sm font-semibold uppercase tracking-[0.12em] text-[#9feaff]">{item.label}</h3>
                  <p className="mt-3 text-sm leading-7 text-white/64">{item.value}</p>
                </Card>
              </Reveal>
            ))}
          </div>
          {service.exampleDiagram || service.exampleCode ? (
            <div className="mt-5 grid gap-5 lg:grid-cols-2">
              {service.exampleDiagram ? (
                <figure className="rounded-2xl border border-white/10 bg-black/20 p-5 sm:p-6">
                  <figcaption className="font-semibold text-white">{service.exampleDiagram.title}</figcaption>
                  <div className="mt-5 grid gap-3" aria-label={`${service.exampleDiagram.inputs.join(", ")} connect to ${service.exampleDiagram.output}`}>
                    <ul className="grid gap-2 sm:grid-cols-2">
                      {service.exampleDiagram.inputs.map((input) => <li key={input} className="rounded-lg border border-[#16c8ff]/25 bg-[#16c8ff]/5 px-4 py-3 text-sm text-white/72">{input}</li>)}
                    </ul>
                    <span aria-hidden className="text-center text-xl text-[#67e8f9]">↓</span>
                    <div className="rounded-lg border border-[#9a6dff]/35 bg-[#9a6dff]/8 px-4 py-3 text-center text-sm font-semibold text-white">{service.exampleDiagram.output}</div>
                  </div>
                  <p className="mt-4 text-sm leading-7 text-white/58">{service.exampleDiagram.description}</p>
                </figure>
              ) : null}
              {service.exampleCode ? (
                <figure className="min-w-0 rounded-2xl border border-white/10 bg-black/20 p-5 sm:p-6">
                  <figcaption className="font-semibold text-white">{service.exampleCode.title}</figcaption>
                  <pre className="mt-5 overflow-x-auto rounded-xl border border-white/10 bg-[#050611] p-4 text-sm leading-7 text-[#c8f4ff]"><code>{service.exampleCode.code}</code></pre>
                  <p className="mt-4 text-sm leading-7 text-white/58">{service.exampleCode.description}</p>
                </figure>
              ) : null}
            </div>
          ) : null}

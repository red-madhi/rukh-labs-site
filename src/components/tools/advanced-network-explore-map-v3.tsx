...
/** Radius in VIEWBOX pixels. The outer graph transform is divided back out later,
 * so nodes stay readable while zoom increases the space between them. */
function nodeScreenRadius(node: PositionedNode) {
  if (node.kind === "self") return 42;
  if (node.kind === "target") return 38;
  if (node.isBridge) return 32;
  if (node.kind === "mutual") return 25;
  return 21;
}
...
function buildPositions(graph: ExploreGraph): PositionedNode[] {
  ...
  if (self) {
    result.push({ ...self, x: CENTER_X, y: CENTER_Y, isBridge: false, targetIds: [] });
  }
  ...
  targets.forEach((node) => {
    ...
    result.push({ ...node, ...point, isBridge: false, targetIds: [] });
  });

  return result;
}

export function AdvancedNetworkExploreMapV3({
  scope,
  targetHandles,
}: {
  scope: StartingNetworkScope;
  ...
  const pickNodeAt = useCallback(
    (clientX: number, clientY: number) => {
      ...
      for (const node of positioned) {
        const viewX = transform.x + node.x * transform.scale;
        const viewY = transform.y + node.y * transform.scale;
        const screenX = rect.left + (viewX / VIEW_W) * rect.width;
        const screenY = rect.top + (viewY / VIEW_H) * rect.height;
        const distance = Math.hypot(clientX - screenX, clientY - screenY);
        const renderedRadius = nodeScreenRadius(node) * (rect.width / VIEW_W);
        const hitRadius = Math.max(30, renderedRadius + 15);
  ...
  function focusNode(node: PositionedNode) {
    const scale = node.kind === "target" ? 4.8 : node.kind === "self" ? 3.8 : 6.4;
    setTransform({
      scale,
      x: VIEW_W / 2 - node.x * scale,
      y: VIEW_H * 0.41 - node.y * scale,
    });
  ...
  function endPointer(event: React.PointerEvent<SVGSVGElement>) {
    ...
    const movedPx = tracked
      ? Math.hypot(event.clientX - tracked.startClientX, event.clientY - tracked.startClientY)
      : Number.POSITIVE_INFINITY;

    if (wasSinglePointer && !pinchingRef.current && movedPx <= TAP_SLOP_PX) {
      setSelectedId(pickNodeAt(event.clientX, event.clientY));
    }
  ...
  function onWheel(event: React.WheelEvent<SVGSVGElement>) {
    ...
    zoomAt(transform.scale * (event.deltaY < 0 ? 1.18 : 0.845), point.x, point.y);
  }

  if (!oauth.did) return null;

  const labelsForAll = transform.scale >= 1.45;
  const labelsForBridges = transform.scale >= 0.75;
  const deepZoom = transform.scale >= 3;
    ...
    <div className="min-w-0 max-w-full overflow-hidden rounded-2xl border border-white/10 bg-[#03060a]">
      <div className="flex min-w-0 flex-col gap-3 border-b border-white/8 bg-black/30 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[11px] leading-5 text-white/46">
            <strong className="text-white/70">Tap a person for details.</strong>{" "}
            Pinch to spread the network apart; people stay readable instead of growing into giant bubbles.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button
      ...
      {!graph ? (
        ...
        <div className="relative w-full overflow-hidden">
          ...
          >
            <defs>
              <pattern id="explore-v3-grid" width="36" height="36" patternUnits="userSpaceOnUse">
                <path d="M36 0H0V36" fill="none" stroke="#ffffff" strokeOpacity="0.022" />
              </pattern>
              <radialGradient id="explore-v3-core-halo">
            ...
            <rect width={VIEW_W} height={VIEW_H} fill="url(#explore-v3-grid)" />

            <g transform={`translate(${transform.x} ${transform.y}) scale(${transform.scale})`}>
              <circle cx={CENTER_X} cy={CENTER_Y} r="430" fill="url(#explore-v3-core-halo)" />
              <circle cx={CENTER_X} cy={CENTER_Y} r="190" fill="none" stroke="#16c8ff" strokeOpacity="0.045" strokeWidth={1.5 / transform.scale} />
              <circle cx={CENTER_X} cy={CENTER_Y} r="350" fill="none" stroke="#16c8ff" strokeOpacity="0.028" strokeWidth={1.5 / transform.scale} />
              ...
              {graph.edges.map((edge, index) => {
                ...
                if (deepZoom && !selectedId && peer && !edge.reciprocal) return null;
                ...
                return (
                  <line
                    ...
                    y2={target.y}
                    stroke={color}
                    strokeWidth={(bridge ? 2.2 : peer ? 1 : 1.35) / transform.scale}
                    strokeOpacity={active ? baseOpacity : 0.012}
                  />
                );
              })}
              ...
              {positioned.map((node) => {
                ...
                const screenR = nodeScreenRadius(node);
                const r = screenR / transform.scale;
                const labelVisible =
                  node.kind === "self" ||
                  node.kind === "target" ||
                  isSelected ||
                  ...
                  labelsForAll;
                const labelFont = (isSelected ? 26 : 23) / transform.scale;
                const labelWidth = (isSelected ? 310 : 270) / transform.scale;
                const labelHeight = 38 / transform.scale;
                ...
                return (
                  ...
                  >
                    {node.kind === "target" ? (
                      <circle r={(screenR * 1.9) / transform.scale} fill="#aa63ff" fillOpacity="0.055" />
                    ) : node.isBridge ? (
                      <circle r={(screenR * 1.75) / transform.scale} fill="#e6bd73" fillOpacity="0.045" />
                    ) : null}

                    {isSelected ? (
                      <circle
                        r={(screenR + 12) / transform.scale}
                        fill="none"
                    ...
                    <circle
                      ...
                      fill={fillFor(node)}
                      stroke={strokeFor(node)}
                      strokeWidth={(node.kind === "self" || node.kind === "target" || node.isBridge ? 3 : 2) / transform.scale}
                    />
                    ...
                    {labelVisible ? (
                      <g transform={`translate(0 ${(screenR + 30) / transform.scale})`}>
                        <rect
                          x={-labelWidth / 2}
                          y={-labelHeight / 2}
                          width={labelWidth}
                          height={labelHeight}
                          rx={11 / transform.scale}
                          fill="#03060a"
                          fillOpacity="0.96"
                          stroke={strokeFor(node)}
                        ...
                        <text
                          textAnchor="middle"
                          dominantBaseline="middle"
                          fill={node.kind === "target" ? "#eadcff" : node.isBridge ? "#ffe0a3" : "#e4f3f7"}
                          fontSize={labelFont}
                          fontWeight={node.kind === "target" || node.isBridge || isSelected ? "700" : "600"}
                        >
                          @{node.handle.length > 26 ? `${node.handle.slice(0, 25)}…` : node.handle}
                        </text>
                      </g>
                    ) : null}
                  </g>
          ...
          <div className="pointer-events-none absolute left-3 top-3 z-20 flex flex-col gap-2">
            <div className="pointer-events-auto flex items-center gap-1 rounded-xl border border-white/10 bg-[#05080c]/94 p-1 shadow-xl backdrop-blur-md">
              <button type="button" onClick={() => zoomAt(transform.scale * 1.5)} className="grid size-10 place-items-center rounded-lg text-white/70 active:bg-white/10" aria-label="Zoom in">
                <Plus className="size-4" aria-hidden />
              </button>
              <button type="button" onClick={() => zoomAt(transform.scale / 1.5)} className="grid size-10 place-items-center rounded-lg text-white/70 active:bg-white/10" aria-label="Zoom out">
                <Minus className="size-4" aria-hidden />
              </button>
              <button type="button" onClick={resetView} className="grid size-10 place-items-center rounded-lg text-white/70 active:bg-white/10" aria-label="Reset map">
          ...
          {!selected ? (
            <div className="pointer-events-none absolute inset-x-3 bottom-3 z-20 rounded-xl border border-white/10 bg-[#05080c]/92 px-4 py-3 text-center text-[11px] font-medium text-white/55 backdrop-blur-md sm:left-1/2 sm:right-auto sm:w-auto sm:-translate-x-1/2">
              Tap any person to open their account card.
            </div>
          ) : (
            <div className="absolute inset-x-2 bottom-2 z-40 max-h-[46%] overflow-y-auto rounded-2xl border border-white/14 bg-[#070a0e]/98 p-4 shadow-2xl backdrop-blur-xl sm:inset-x-auto sm:bottom-4 sm:right-4 sm:w-[370px]">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[9px] font-semibold uppercase tracking-[0.12em] text-[#8ce8ff]">{roleLabel(selected)}</p>
                  <p className="mt-1 break-words text-base font-semibold text-white">{selected.displayName || `@${selected.handle}`}</p>
                  <p className="mt-1 break-all text-[11px] text-white/42">@{selected.handle}</p>
                </div>
                <button type="button" onClick={() => setSelectedId(null)} className="grid size-9 shrink-0 place-items-center rounded-lg border border-white/9 text-white/55" aria-label="Close account details">
              ...
              {selected.targetIds.length ? (
                <div className="mt-3">
                  <p className="text-[9px] font-semibold uppercase tracking-[0.1em] text-white/30">Connects toward</p>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {selected.targetIds.slice(0, 6).map((targetId) => {
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button type="button" onClick={() => focusNode(selected)} className="inline-flex items-center gap-1.5 rounded-lg border border-[#16c8ff]/20 bg-[#16c8ff]/[0.045] px-3 py-2 text-[11px] font-medium text-[#9cecff]">
                  <Focus className="size-3.5" aria-hidden />
                  Center on person
                </button>
                <a href={`https://bsky.app/profile/${selected.handle}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-white/9 bg-white/[0.025] px-3 py-2 text-[11px] font-medium text-white/60">
                  Open profile
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-2 border-t border-white/8 bg-[#05080b] px-4 py-3 text-[10px] text-white/42">

...
function dataNodeId(target: EventTarget | null) {
  const element = target instanceof Element ? target : null;
  return element?.closest?.("[data-network-node-id]")?.getAttribute("data-network-node-id") ?? null;
}

export function AdvancedNetworkExploreMapV4({
  scope,
  targetHandles,
}: {
  scope: StartingNetworkScope;
  ...
  const clientToView = useCallback((clientX: number, clientY: number) => {
    ...
    if (!matrix) return { x: VIEW_W / 2, y: VIEW_H / 2 };
  ...
  function beginPointer(event: ReactPointerEvent<SVGSVGElement>) {
    ...
    const pendingNodeId = dataNodeId(event.target);
  ...
  function movePointer(event: ReactPointerEvent<SVGSVGElement>) {
    ...
    const svg = svgRef.current;
    if (!svg) return;
    const start = clientToView(pan.clientX, pan.clientY);
    const now = clientToView(event.clientX, event.clientY);
    setTransform((current) => ({
      ...current,
      x: pan.x + (now.x - start.x),
      y: pan.y + (now.y - start.y),
    }));
  }
  ...
  function endPointer(event: ReactPointerEvent<SVGSVGElement>) {
    ...
    if (
      ...
      !pinchingRef.current &&
      movedPx <= TAP_SLOP_PX &&
      tracked?.pendingNodeId &&
      byId.has(tracked.pendingNodeId)
    ) {
      setSelectedId(tracked.pendingNodeId);
    }
      ...
      <div className="flex min-w-0 flex-col gap-3 border-b border-white/8 bg-black/30 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[11px] leading-5 text-white/46">
            <strong className="text-white/70">Tap the circle or the name.</strong>{" "}
            Each person has a large invisible touch target; pinch zoom and one-finger pan still work normally.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button
      ...
      {!graph ? (
        ...
        <div className="relative w-full overflow-hidden">
          <svg
            ...
            className="block h-[74svh] min-h-[580px] max-h-[840px] w-full select-none sm:h-[700px]"
            style={{ touchAction: "none" }}
            role="img"
            aria-label="Interactive Bluesky network map. Tap a circle or name for account details, drag to pan, pinch to zoom."
            onWheel={onWheel}
          ...
          >
            <defs>
              <pattern id="explore-v4-grid" width="36" height="36" patternUnits="userSpaceOnUse">
                <path d="M36 0H0V36" fill="none" stroke="#ffffff" strokeOpacity="0.022" />
              </pattern>
              <radialGradient id="explore-v4-core-halo">
            ...
            <rect width={VIEW_W} height={VIEW_H} fill="url(#explore-v4-grid)" />

            <g transform={`translate(${transform.x} ${transform.y}) scale(${transform.scale})`}>
              <circle cx={CENTER_X} cy={CENTER_Y} r="430" fill="url(#explore-v4-core-halo)" pointerEvents="none" />
              ...
              {graph.edges.map((edge, index) => {
                  ...
                  : edge.kind === "self-target"
                    ...
                    : peer
                      ...
                      : edge.kind === "self-mutual"
                        ? "#55d7ff"
                        : "#667785";
                const opacity = bridge ? (deepZoom ? 0.38 : 0.7) : peer ? (deepZoom ? 0.04 : 0.1) : deepZoom ? 0.1 : 0.22;
                ...
                return (
                  <line
                    ...
                    strokeOpacity={active ? opacity : 0.01}
                    pointerEvents="none"
                  />
                );
              })}
              ...
              {positioned.map((node) => {
                ...
                const labelVisible =
                  ...
                  labelsForAll;
                const labelW = 292 / transform.scale;
                const labelH = 44 / transform.scale;
                const labelY = (screenR + 31) / transform.scale;
                const hitR = 92 / transform.scale;

                return (
                  <g key={node.id} transform={`translate(${node.x} ${node.y})`} opacity={related ? 1 : 0.13}>
                    <circle
                    ...
                    {node.kind === "target" ? (
                      <circle r={(screenR * 1.9) / transform.scale} fill="#aa63ff" fillOpacity="0.055" pointerEvents="none" />
                    ) : node.isBridge ? (
                      <circle r={(screenR * 1.75) / transform.scale} fill="#e6bd73" fillOpacity="0.045" pointerEvents="none" />
                    ) : null}

                    {isSelected ? (
                      <circle
                        r={(screenR + 14) / transform.scale}
                        fill="none"
                        stroke={strokeFor(node)}
                        strokeWidth={3.2 / transform.scale}
                        strokeOpacity="1"
                        pointerEvents="none"
                      />
                    ) : null}
                    ...
                    {labelVisible ? (
                      <g transform={`translate(0 ${labelY})`}>
                        <rect
                          ...
                          width={labelW}
                          height={labelH}
                          rx={12 / transform.scale}
                          fill="#03060a"
                          fillOpacity="0.97"
                          stroke={strokeFor(node)}
                          strokeOpacity={isSelected ? 0.6 : 0.28}
                          strokeWidth={1.3 / transform.scale}
                          pointerEvents="all"
                        />
                        <text
                          data-network-node-id={node.id}
                          ...
                          fontSize={(isSelected ? 27 : 24) / transform.scale}
          ...
          {!selected ? (
            <div className="pointer-events-none absolute inset-x-3 bottom-3 z-20 rounded-xl border border-white/10 bg-[#05080c]/92 px-4 py-3 text-center text-[11px] font-medium text-white/60 backdrop-blur-md">
              Tap a circle or its name — the whole area is selectable.
            </div>
          ) : (
            <div className="absolute inset-x-2 bottom-2 z-40 max-h-[50%] overflow-y-auto rounded-2xl border border-[#16c8ff]/22 bg-[#070a0e]/99 p-4 shadow-2xl backdrop-blur-xl sm:inset-x-auto sm:bottom-4 sm:right-4 sm:w-[380px]">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[9px] font-semibold uppercase tracking-[0.12em] text-[#8ce8ff]">{roleLabel(selected)}</p>
                  <p className="mt-1 break-words text-lg font-semibold text-white">{selected.displayName || `@${selected.handle}`}</p>
                  <p className="mt-1 break-all text-[11px] text-white/46">@{selected.handle}</p>
                </div>

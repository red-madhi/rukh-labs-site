...
export function ContactForm({
  ...
  const validDesign = designDirections.some(
  ...
  function updateReason(reason: string) {
    setForm((current) => ({
      ...
      reason,
      projectType:
        reason === CAREER_REASON ? careerProjectTypes[0] : websiteProjectTypes[0],
  ...
  function openEmailDraft() {
    const websitePackageLabel =
      websitePackages.find((websitePackage) => websitePackage.id === form.packageId)
        ?.name ?? form.packageId;
    const careerPackageLabel =
      careerPortfolioPackages.find((careerPackage) => careerPackage.id === form.packageId)
        ?.name ?? form.packageId;
    const designLabel =
      designDirections.find((direction) => direction.slug === form.designDirection)
        ?.name ?? form.designDirection;

    let subject = form.reason + " — " + form.name.trim();
    let lines = [
    ...
    if (isWebsiteProject) {
      ...
      lines = [
        ...
        "Package interest: " + websitePackageLabel,
    ...
    if (isCareerProject) {
      ...
      lines = [
        ...
        "Email: " + form.email.trim(),
        "Phone: " + (form.phone.trim() || "Not provided"),
        "Current or target role: " + (form.organization.trim() || "Not provided"),
        "",
        "Project type: " + form.projectType,
        "Package interest: " + careerPackageLabel,
        "Budget: " + form.budget,
        "Preferred timeline: " + form.timeline,
        "",
        "TARGET ROLES / PORTFOLIO GOALS",
        form.message.trim(),
        "",
        "How they found Rukh Labs: " + (form.referral.trim() || "Not provided"),
      ];
  ...
  const projectTypeOptions = isCareerProject
    ? careerProjectTypes
    : websiteProjectTypes;
  const packageOptions = isCareerProject
      ...
      >
          ...
          <>
            ...
            <div>
              <label htmlFor="contact-organization" className={labelStyles}>
                {isCareerProject ? "Current or target role" : "Business, brand, or project name"}{" "}
              ...
              <input
                ...
                placeholder={
                  isCareerProject
                    ? "What role are you in or pursuing?"
                ...
                >
                  ...
                  {packageOptions.map((projectPackage) => (
                    <option key={projectPackage.id} value={projectPackage.id}>
                      {projectPackage.name} — {projectPackage.price}
                    </option>
                  ))}
                </select>
              </div>
        ...
        <div>
          <label htmlFor="contact-message" className={labelStyles}>
            {isCareerProject
              ? "What should the portfolio prove?"
              : isWebsiteProject
          ...
          <textarea
            ...
            placeholder={
              isCareerProject
                ? "Describe the roles you are targeting, the experience or projects you can show, privacy constraints, and what would make the portfolio successful."
                : isWebsiteProject
        ...
        {error ? (
          <p
            role="alert"
            className="rounded-lg border border-[#ff7f73]/20 bg-[#ff7f73]/8 px-4 py-3 text-sm text-[#ffc0b8]"
          >
            {error}
          </p>
        ) : null}
